...
import { fromInjected, fromConnection, ephemeral } from "@openzeppelin/network";
...
class Main extends Component {
  ...
  async componentDidMount() {
    ...
    lib.eth
      .subscribe("logs", { address: contract._address })
      .on("data", (log) => {
        this.handleLog(log);
      })
    ...
    //   web3.eth
    //     .subscribe("logs", { address: instance.address })
    //     .on("data", (log) => {
    //       this.handleLog(log);
    //     })
    //     .on("error", (err) => console.log(err));
  ...
  handleSendApi = async () => {
    ...
    if (this.state.val > 0) {
      ...
      try {
        await contract.methods.set(this.state.val).send({ from: accounts[0] });
        // const result = await axios.post("/eth/set", {
        //   from: accounts[0],
        //   val: this.state.val,
        // });
        // if (
  ...
  //         const result = await axios.post("/eth/setTx", {
  //           from: "0x6C1b86657256A2Bb548B11EE332049451ce5ca93",
  //           val: this.state.val,
  //         });
  ...
  handleLog = (log) => {
    ...
    const returnValues = lib.eth.abi.decodeLog(params, log.data);
